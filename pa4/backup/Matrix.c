...
typedef struct MatrixObj{
    ...
    int nnz; //the number of non-zero element in M
}MatrixObj;
...
Matrix newMatrix(int n){
    ...
    M -> nnz = 0; //the non-zero elemenet of matrix
    M -> row = calloc(n+1,sizeof(List)); //allocate memory fot the row
    int i;
    for(i = 0; i <= n; i++){
...
Entry newEntry(int a, double b){
    ...
    Entry E = (EntryObj *)malloc(1 * sizeof(EntryObj));//used malloc to set the entry
    E -> column = a;
    E -> value = b;
    return E;
}
...
int NNZ(Matrix M){
    ...
    return M->nnz; //return the non-zero element of the matrix
}
...
void makeZero(Matrix M){
    ...
    for(int i = 0; i < M->size; i++){
        List a = M->row[i];
        while(length(a) > 0){ //if the length of a not equal 0
            deleteFront(a);
        }
    }
    M->nnz = 0;
